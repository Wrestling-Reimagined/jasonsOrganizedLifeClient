-- Personal Life Manager manual bootstrap script
-- Safe to run multiple times (idempotent where practical).
-- Run against the target schema (for Railway this is usually `railway`).
...
-- Incremental schema updates (safe for existing databases).
ALTER TABLE meal_consumptions
  ADD COLUMN IF NOT EXISTS rating TINYINT UNSIGNED NOT NULL DEFAULT 5;
...
-- Keep migration tracker aligned with app runner.
INSERT IGNORE INTO _migrations (name) VALUES
  ('001_init.sql'),
  ('002_add_meal_ratings.sql'),
  ('003_add_meal_expiration.sql'),
  ('004_payments_datetime.sql');
